...
import lidiff.models.minkunet as lidiffminknet
...
class DiffCompletion(LightningModule):
    def __init__(self, config, full_exp_dir):
        ...
        self.model_refine = lidiffminknet.MinkUNet(in_channels=3, out_channels=3*6)
        ckpt_refine = torch.load(config['refine'])
...
def load_pcd_from_bytes(pcd_bytes, filename):
    temp_dir = "temp_files"
    ...
    try:
        if temp_path.endswith('.bin'):
            points = np.fromfile(temp_path, dtype=np.float32).reshape((-1, 4))[:, :3]
        elif temp_path.endswith('.ply') or temp_path.endswith('.pcd'):
