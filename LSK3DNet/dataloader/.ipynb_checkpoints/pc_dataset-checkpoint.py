...
class SemKITTI_sk(data.Dataset):
    ...
    def __getitem__(self, index):
        # Load raw data as (X, 3) since intensity is dropped
        raw_data = np.fromfile(self.im_idx[index], dtype=np.float32).reshape((-1, 3))  # Changed from (-1, 4)
        xyz = raw_data  # All 3 columns are coordinates
        feat = np.zeros((len(raw_data), 1), dtype=np.float32)  # Signal set to zeros (no intensity)
        ...
        data_dict['signal'] = feat  # No intensity, set to zeros
        data_dict['origin_len'] = origin_len

        return data_dict, self.im_idx[index]

@register_dataset
class PCD_Single(data.Dataset):
    """
    Single-.pcd reader that mimics SemKITTI_* interface:
      __init__(data_path, imageset='test', label_mapping=..., num_vote=1)
    Returns (data_dict, path) with keys: xyz, labels, instance_label, signal, origin_len
    """
    def __init__(self, data_path, imageset='test', label_mapping=None, num_vote=1):
        self.imageset = imageset
        self.num_vote = int(num_vote) if num_vote is not None else 1

        # keep parity with SemKITTI_* (not used for test)
        self.learning_map = None
        if label_mapping and os.path.isfile(label_mapping):
            with open(label_mapping, 'r') as f:
                y = yaml.safe_load(f)
            self.learning_map = y.get('learning_map')

        if not (os.path.isfile(data_path) and data_path.lower().endswith('.pcd')):
            raise ValueError(f"PCD_Single_O3D expects a .pcd file path, got: {data_path}")

        # replicate for num_vote to satisfy callers that iterate that many times
        self.im_idx = [data_path] * self.num_vote

        self.downsample = SphereCrop(point_max=self.cfg['d_point_num'], mode="random")

    def __len__(self):
        return len(self.im_idx)

    def _read_pcd(self, path):
        # Prefer tensor API (keeps custom attributes like 'intensity')
        try:
            tpcd = o3d.t.io.read_point_cloud(path)
            xyz = tpcd.point['positions'].numpy().astype(np.float32)  # (N,3)
            sig = np.zeros((xyz.shape[0], 1), dtype=np.float32)  # No intensity, set to zeros
            xyz = self.downsample(xyz)
            return xyz, sig
        except Exception:
            # Legacy fallback (attributes often lost)
            pcd = o3d.io.read_point_cloud(path)
            xyz = np.asarray(pcd.points, dtype=np.float32)  # (N,3)
            sig = np.zeros((xyz.shape[0], 1), dtype=np.float32)  # No intensity
            xyz = self.downsample(xyz)
            return xyz, sig

    def __getitem__(self, index):
        p = self.im_idx[index]
        xyz, feat = self._read_pcd(p)
        n = xyz.shape[0]

        # no labels for a lone PCD
        sem  = np.zeros((n, 1), dtype=np.uint8)
        inst = np.zeros((n, 1), dtype=np.uint32)

        return {
            'xyz': xyz,
            'labels': sem,
            'instance_label': inst,
            'signal': feat,  # No intensity, set to zeros
            'origin_len': n
        }, p
...
class nuScenes(data.Dataset):
    ...
    def loadDataByIndex(self, index):
        ...
        raw_data = np.fromfile(lidar_path, dtype=np.float32).reshape((-1, 4))  # Original shape

        # Adjust raw_data to (X, 3) by dropping intensity
        raw_data = raw_data[:, :3]  # Keep only [x, y, z]
    ...
    def __getitem__(self, index):
        ...
        data_dict = {}
        data_dict['xyz'] = pointcloud[:, :3]  # Already (X, 3)
        data_dict['labels'] = sem_label.astype(np.uint8)
        data_dict['instance_label'] = instance_label
        data_dict['signal'] = np.zeros((pointcloud.shape[0], 1), dtype=np.float32)  # No intensity
